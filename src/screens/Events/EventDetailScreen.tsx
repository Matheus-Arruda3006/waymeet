...
export const EventDetailScreen: React.FC<{ route: any; navigation: any }> = ({ route, navigation }) => {
    ...
    const isAttending = event.attendees.some((a: any) => a.id === user?.id);

    const handleJoinLeave = () => {
        if (isAttending) {
            leaveEvent(event.id, user?.id || '');
            addSystemMessage(event.id, `${user?.displayName} saiu do grupo`);
        } else {
            joinEvent(event.id, user?.id || '');
            addSystemMessage(event.id, `${user?.displayName} entrou no grupo`);
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 12 }]}>
                ...
                >
                    <Ionicons
                        name={isAttending ? 'exit-outline' : 'people-outline'}
                        size={20}
                        color={Colors.textInverse}
                    />
                    <Text style={styles.joinButtonText}>
                        {isAttending ? 'Sair do evento' : 'Participar'}
                    </Text>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    joinButton: {
        ...
        gap: 8, backgroundColor: Colors.primary, paddingVertical: 16,
        borderRadius: BorderRadius.xl, ...Shadows.medium,
    },
    leaveButton: { backgroundColor: Colors.error },
    joinButtonText: { fontSize: 16, fontWeight: '700', color: Colors.textInverse },
    chatButton: {
